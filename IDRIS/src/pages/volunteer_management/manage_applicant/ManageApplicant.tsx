...
import { Button, Breadcrumb, Input, Spin, message, Modal, Descriptions, Divider, Tag, Empty } from 'antd';
...
const ManageApplicant: React.FC = () => {
  ...
  useEffect(() => {
    const fetchVolunteers = async () => {
      try {
        ...
        const firstVisible = allData.find(v => statusOf(v) !== 'approved') ?? allData[0] ?? null;
  ...
  // hide approved
  const visibleVolunteers: Volunteer[] = volunteers.filter(v => statusOf(v) !== 'approved');
  ...
  const pickNextVisible = (currentId: number): Volunteer | null => {
    const next = volunteers.find(v => v.volunteer_id !== currentId && statusOf(v) !== 'approved');
    return next ?? null;
  };
  ...
  const acceptVolunteer = async (volunteer: Volunteer) => {
    ...
    // optimistic: mark as approved (it disappears from table due to filter)
