...
import { createProgram, listPrograms, createAssignment as apiCreateAssignment } from '../../../API_Handler/assignment_handler';
...
function toIsoOrNull(dtLocal: string) {
  // dtLocal like "2025-09-07T09:00" -> ISO
...
function isoToDateOnly(iso: string) {
  // "2025-09-07T01:00:00.000Z" -> "2025-09-07" (for legacy servers/types)
...
async function createAssignment(taskId: string | number, volunteerId: string) {
  ...
  // 👇 include task_id in the body to satisfy the backend validator
...
/**
 * Fallback: directly patch availability (used on unassign for now).
 * NOTE: Replace with a proper DELETE /assignment/assignments/{assignmentId}
 * when your backend returns assignment IDs.
 */
async function updateAvailabilityStatus(volunteerId: string, availability: Availability) {
  const id = rawId(volunteerId);
  const endpoint = isOrgId(volunteerId)
    ? `/organization_volunteer/${id}/status`
    : `/volunteer/${id}/status`;
...
const VolunteerAssignmentPage: React.FC = () => {
  ...
  const [newArea, setNewArea] = useState<NewAreaForm>({
    ...
    description: '',
    location: '',
    start_at: new Date().toISOString().slice(0, 16),                 // yyyy-MM-ddTHH:mm (local)
    end_at: new Date(Date.now() + 2 * 60 * 60 * 1000).toISOString().slice(0, 16), // +2h
    maxVolunteers: '',
    requiredSkills: [],
    skillInput: '',
  });
  ...
  const handleAssignVolunteers = async () => {
    ...
    // Try assign each volunteer; accept partial success
    const results = await Promise.allSettled(
      selectedVolunteers.map(async (id) => ({ id, payload: await createAssignment(selectedArea, id) }))
    );
  ...
  const handleRemoveVolunteer = async (areaId: string, volunteerId: string) => {
    // TODO(back-end): delete assignment by volunteer (or return assignment ids in list)
    // Fallback for now: flip availability back to 'available'
    try {
      await updateAvailabilityStatus(volunteerId, 'available');
    } catch (e) {
      console.error(e);
      alert('Unassign failed on server. Please implement DELETE /assignment/assignments/{id}.');
      return;
    }
    ...
    // UI: set volunteer back to available so they reappear on the right
    setVolunteers((prev) =>
  ...
  // Fetch volunteer areas (programs/events) from API
  useEffect(() => {
    const fetchPrograms = async () => {
      try {
        ...
        const mappedAreas: VolunteerArea[] = (Array.isArray(data) ? data : []).map((area: any) => {
          ...
          // Prefer new start/end fields; fallback to legacy task_date as all-day
          ...
          // ✅ Robust lifecycle calc (timestamp-based)
          ...
          const assignedIds: string[] = Array.isArray(area.assigned_volunteer_ids)
            ? area.assigned_volunteer_ids.map((n: any) => String(n))
            : [];
          ...
          return {
            ...
            currentVolunteers: Number(area.current_count) || assignedIds.length || 0,
            location: area.location ?? '',
            start_at: startISO,
            end_at: endISO,
            lifecycle,
            assignedVolunteers: assignedIds, // individuals for now; org ids require backend support
          };
        });
  ...
  const handleCreateArea = async () => {
    ...
    // Backward-compatible payload: include legacy task_date while sending start/end.
    const payload = {
