...
router_admin = APIRouter(
    dependencies=[
        Depends(
            RoleChecker(
                ["finance admin", "operations admin", "superuser", "superadmin"]
            )
        )
    ],
)
    ...
    try:
        ...
        if "donor" not in user.roles:
            user.roles = [
                *user.roles,
                "donor",
            ]  # assign new list to ensure SQLAlchemy change tracking
            db.add(user)
            db.commit()
            db.refresh(user)
...
def get_donor_endpoint(donor_id: int, db: Session = Depends(get_db)):
...
def get_donor_by_user_endpoint(user_id: int, db: Session = Depends(get_db)):
...
def count_donors(
    ...
    donor_type: Optional[str] = Query(
        None, description="Filter by Organization or Individual"
    ),
    db: Session = Depends(get_db),
):
...
def get_verified_donors_endpoint(
    ...
    total = sum(
        1 for d in donor_crud.get_all_donors(db, skip=0, limit=10000) if d.is_verified
    )

    return DonorListResponse(donors=donors, total=total, skip=skip, limit=limit)


@router_admin_or_donor.get(
    "/search/name", response_model=List[DonorResponse]
)  # mark used
...
def verify_donor_endpoint(donor_id: int, db: Session = Depends(get_db)):
...
def unverify_donor_endpoint(donor_id: int, db: Session = Depends(get_db)):
