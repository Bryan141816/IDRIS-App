...
@router.get("/response_dashboard/modality_distribution/record_list", response_model=TableResponse)
def get_modality_table(db: Session = Depends(get_db)):
    ...
    reports = db.query(ModalityDistribution).order_by(ModalityDistribution.date_time.desc()).all()
...
@router.post("/response_dashboard/modality_distribution/add_record", response_model = ModalityDistributionOut)
def add_modality_record(modality_report: ModalityDistributionCreate, db: Session = Depends(get_db)):
    return create_modality_distribution_record(db, modality_report);

@router.delete("/response_dashboard/modality_distribution/delete_record/{record_id}", response_model=dict)
...
def update_modality_record(record_id: int, update: ModalityDistributionCreate, db: Session = Depends(get_db)):
