...
def wait_for_database(max_retries=30, retry_interval=2):
    """Wait for database to be ready before running migrations and ensure required extensions"""
    for attempt in range(max_retries):
        try:
            with engine.connect() as conn:
                ...
                required_extensions = ["fuzzystrmatch", "pg_trgm"]
                for ext in required_extensions:
                    if ext not in installed:
                        try:
                            conn.execute(text(f"CREATE EXTENSION IF NOT EXISTS {ext};"))
                            conn.commit()
                            print(f"🔧 Extension '{ext}' installed/ensured.")
                        except Exception as ext_err:
                            print(f"⚠️ Could not create extension '{ext}': {ext_err}")
