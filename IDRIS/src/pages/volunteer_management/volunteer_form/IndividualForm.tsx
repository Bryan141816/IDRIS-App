    ...
    const IndividualForm: React.FC = () => {
        ...
        const onFinish = async (values: IndividualFormValues): Promise<void> => {
            const userId = values.user_id; // Get the user ID from the form data

            const alreadySubmitted = await checkIfAlreadySubmitted(userId);
            if (alreadySubmitted) {
                // Show alert that user has already submitted the form
                Swal.fire({
