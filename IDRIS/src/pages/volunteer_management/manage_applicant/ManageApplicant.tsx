...
import { Button, Breadcrumb, Input, Spin, message, Modal, Descriptions, Divider, Tag, Empty, Image } from 'antd';
...
import { getAllOrganizationVolunteers, updateOrganizationVolunteerStatus } from '../../../API_Handler/organization_volunteer_handler';
import { FilePdfOutlined, FileImageOutlined, DownloadOutlined, FileOutlined } from '@ant-design/icons';
...
const ManageApplicant: React.FC = () => {
        ...
        const fetchVolunteers = async () => {
            try {
                ...
                const extractOrgCertFiles = (org: any): string[] => {
                    if (Array.isArray(org.organization_certificates)) return org.organization_certificates;
                    if (typeof org.organization_certificate === 'string' && org.organization_certificate) return [org.organization_certificate];
                    if (Array.isArray(org.certification_files)) return org.certification_files;
                    if (typeof org.certification_paths === 'string') return splitCsv(org.certification_paths);
                ...
                const mappedIndData: Volunteer[] = (individualData || []).map((iv: any) => ({
                    ...iv,
                    availability_status: (iv.availability_status as Availability) ?? 'unavailable', // ✅ default explicit
                    ind_cert_files:
                ...
                const mappedOrgData: Volunteer[] = (orgData || []).map((org: any) => ({
                    ...org,
                    availability_status: (org.availability_status as Availability) ?? 'unavailable', // ✅ default explicit
                    ...
                    organization_pictures: Array.isArray(org.organization_pictures) ? org.organization_pictures : undefined,
    ...
    // search filter
    const filteredVolunteers: Volunteer[] = visibleVolunteers.filter((v: Volunteer) => {
        ...
        return displayName.includes(searchLower) || email.includes(searchLower) || phone.includes(searchText);
    });

    function getOrgPictures(v?: Volunteer): string[] {
        if (!v || !v.organization_name) return [];
        const pics: string[] = [];
        if (Array.isArray(v.organization_pictures) && v.organization_pictures.length) {
    ...
    const pickNextVisible = (currentId: number): Volunteer | null => {
        const next = volunteers.find(v => v.volunteer_id !== currentId && isVisible(v));
        return next ?? null;
    };
    ...
    const handleViewCredentials = async (): Promise<void> => {
        ...
        // optimistic
        setVolunteers(prev => prev.map(v => (v.volunteer_id === id ? { ...v, status: nextStatus } : v)));
    ...
    const handleDeclineClick = (e: React.MouseEvent<HTMLSpanElement>, volunteer: Volunteer): void => {
        ...
        swalWithCustomButtons
            ...
            .then(async (result: SweetAlertResult) => {
                ...
                // optimistic
                setVolunteers(prev => prev.map(v => (v.volunteer_id === volunteer.volunteer_id ? { ...v, status: 'rejected' } : v)));
                ...
                try {
                    ...
                    // rollback
                    setVolunteers(prev => prev.map(v => (v.volunteer_id === volunteer.volunteer_id ? { ...v, status: prevStatus } : v)));
                    if (selectedVolunteer?.volunteer_id === volunteer.volunteer_id) {
            ...
            <div className="main-content">
                ...
                <div className="applicant-details-card">
                    <h3 className="details-title">
                        {selectedVolunteer ? `Applicant Details - ${getDisplayName(selectedVolunteer)}` : 'Select an applicant to view details'}
                    </h3>
                    ...
                    {selectedVolunteer && (
                        <div className="details-container">
                            <div className="detail-item">
                                ...
                                <p className="detail-value">{selectedVolunteer.email || selectedVolunteer.organization_email || 'N/A'}</p>
                            </div>
                            <div className="detail-item">
                                <p className="detail-label">Contact Number</p>
                                <p className="detail-value">
                                    {selectedVolunteer.phone_number || selectedVolunteer.organization_phone_number || 'N/A'}
                                </p>
                            </div>
                            <div className="detail-item">
                            ...
                            <div className="view-credentials-container">
                                <Button type="primary" className="view-credentials-button" onClick={handleViewCredentials}>
                                    View More Info
                                </Button>
                            </div>
                        </div>
                        ...
                        {selectedVolunteer.organization_name ? (
                            <>
                                <Descriptions title="Organization Details" column={2} bordered size="middle">
                                    ...
                                    <Descriptions.Item label="Applied On">{formatDate(selectedVolunteer.created_at)}</Descriptions.Item>
                                </Descriptions>
                                ...
                                <Descriptions title="Contact Person" column={2} bordered size="middle">
                                    ...
                                    <Descriptions.Item label="Position">{show(selectedVolunteer.contact_person_position)}</Descriptions.Item>
                                    <Descriptions.Item label="Email">{show(selectedVolunteer.contact_person_email)}</Descriptions.Item>
                                    <Descriptions.Item label="Phone">{show(selectedVolunteer.contact_person_phone_number)}</Descriptions.Item>
                                </Descriptions>
                                        ...
                                        if (pics.length === 0) {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <div>
                                                        <div style={{ fontWeight: 500 }}>No organization picture</div>
                                                        <div style={{ fontSize: 12, color: '#888' }}>
                                                            A placeholder is shown until the applicant uploads an image.
                                                        </div>
                                                    </div>
                                                </div>
                                            );
                                                    ...
                                                    {pics.map((p, idx) => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                <Image
                                                                    src={url}
                                                                    alt={name || `Organization picture ${idx + 1}`}
                                                                    width="100%"
                                                                    height={150}
                            ...
                            // INDIVIDUAL VIEW
                            <>
                                <Descriptions title="Personal Details" column={2} bordered size="middle">
                                    <Descriptions.Item label="Full Name" span={2}>
                                        {getFullNamewMid(selectedVolunteer)}
                                    </Descriptions.Item>
                                    <Descriptions.Item label="Email">{selectedVolunteer.email || 'N/A'}</Descriptions.Item>
                                    <Descriptions.Item label="Contact Number">{selectedVolunteer.phone_number || 'N/A'}</Descriptions.Item>
                                    <Descriptions.Item label="Date of Birth">{formatDate(selectedVolunteer.birthday)}</Descriptions.Item>
                                    <Descriptions.Item label="Age">{selectedVolunteer.age ?? 'N/A'}</Descriptions.Item>
                                    <Descriptions.Item label="Gender">{selectedVolunteer.gender || 'N/A'}</Descriptions.Item>
                                ...
                                <Descriptions title="Medical Information" column={1} bordered size="middle">
                                    <Descriptions.Item label="Medical Conditions">{selectedVolunteer.medical_conditions || 'None'}</Descriptions.Item>
                                    {selectedVolunteer.other_medical_conditions && (
                                        <Descriptions.Item label="Other Medical Conditions">{selectedVolunteer.other_medical_conditions}</Descriptions.Item>
                                    )}
                                </Descriptions>
                            </>
                        )}
