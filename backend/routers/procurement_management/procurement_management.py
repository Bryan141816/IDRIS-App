...
def get_dashboard(db: Session = Depends(get_db)):
    counts = db.query(
        ...
        func.sum(case((ProcurementRequest.status == "Approved", 1), else_=0)).label(
            "approved"
        ),
        func.sum(case((ProcurementRequest.status != "Approved", 1), else_=0)).label(
            "not_approved"
    ...
    logs = (
        db.query(ProcurementRequestLog)
...
def approve_reject_request(
    ...
    reason: Optional[str] = Query(None),  # <-- ADD THIS
):
    ...
    try:
        ...
        if query.request_type == "relief":
            ...
            procurement_items = (
                ...
                .filter(ProcurementRequestItem.request_id == query.request_id)
                .all()
            )
